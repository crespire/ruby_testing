...
# or 3 different sentences so that everything could be clearly understood and
# in this case if a method does many different things it can be difficult to
# test.
...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continue) { described_class.new(5, '1') }
      # remove the 'x' before running this test
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_three) { described_class.new(5, '5', 9) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        check_phrase = "That was hard. It took you 9 guesses!\n"
        expect { game_three.final_message }.to output(check_phrase).to_stdout
      end
    end
  end
end
