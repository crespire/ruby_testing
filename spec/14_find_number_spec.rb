...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    subject(:game_guessing) { described_class.new(min, max, number_guessing) }
  ...
  describe '#game_over?' do
    ...
    let(:max) { 9 }
    let(:guess) { 6 }
    let(:number_over) { double('random_number', value: guess) }

    subject(:over_test) { described_class.new(min, max, number_over, guess) }
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        expect(over_test).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:number_over) { double('random_number', value: 3) }

      it 'is not game over' do
        expect(over_test).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { edge_guess_game.update_range }.to change { edge_guess_game.min }.from(5).to(8)
      end
